...
ffModel = DoubleJointedArmFeedforward(
  ...
  JointConfig(8.0, L[1] + L[2], 1, (L[1] + L[2]) / 1.5, DCMotor.getNEO(1).withReduction(50.0))
)
wrist_max_v = 10
wrist_max_a = 20
...
for i in range(n):
    ...
    opti.set_initial(theta[0], (opti.value(end_theta[0]) - opti.value(start_theta[0])) * ((i + 1) / (n + 2)) + opti.value(start_theta[0]))
    opti.set_initial(theta[1],(opti.value(end_theta[1]) - opti.value(start_theta[1])) * ((i + 1) / (n + 2)) + opti.value(start_theta[1]))
    opti.set_initial(theta[2],(opti.value(end_theta[2]) - opti.value(start_theta[2])) * ((i + 1) / (n + 2)) + opti.value(start_theta[2]))
...
for i in range(n + 2):
  ...
  if i != 0 and i != len(theta_points) - 1:
    wrist_base = (
      L[0] * cos(current_theta[0]) + L[1] * cos(current_theta[0] + current_theta[1]),
      L[0] * sin(current_theta[0]) + L[1] * sin(current_theta[0] + current_theta[1])
    )
    wrist_tip = (
      L[0] * cos(current_theta[0]) + L[1] * cos(current_theta[0] + current_theta[1]) + L[2] * cos(current_theta[0] + current_theta[1] + current_theta[2]),
      L[0] * sin(current_theta[0]) + L[1] * sin(current_theta[0] + current_theta[1]) + L[2] * sin(current_theta[0] + current_theta[1] + current_theta[2])
    )
...
current_time = 0
for i in range(n + 2):
    animation_points.append((current_time, opti.value(theta_points[i][0]), opti.value(theta_points[i][1]), opti.value(theta_points[i][2])))
...
def animate(i):
    ...
    x = [
      ...
      L[0] * cos(theta_1) + L[1] * cos(theta_1 + theta_2) + L[2] * cos(theta_1 + theta_2 + theta_3)
    ]
    y = [
      0,
      L[0] * sin(theta_1),
      L[0] * sin(theta_1) + L[1] * sin(theta_1 + theta_2),
      L[0] * sin(theta_1) + L[1] * sin(theta_1 + theta_2) + L[2] * sin(theta_1 + theta_2 + theta_3)
    ]
    ax.clear()
    ax.plot(x, y)
    ax.plot([-2, 2], [-0.1, -0.1])
