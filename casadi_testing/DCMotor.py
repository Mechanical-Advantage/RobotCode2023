...
class DCMotor:
  ...
  def __init__(self, nominalVoltageVolts: float, stallTorqueNewtonMeters: float, stallCurrentAmps: float, freeCurrentAmps: float, freeSpeedRadPerSec: float, numMotors: int):
    ...
    self._KvRadPerSecPerVolt = freeSpeedRadPerSec / (nominalVoltageVolts - self._rOhms * self._freeCurrentAmps)
    self._KtNMPerAmp = self._stallTorqueNewtonMeters / self._stallCurrentAmps

  def getVoltage(self, torqueNm, speedRadiansPerSec):
    return 1.0 / self._KvRadPerSecPerVolt * speedRadiansPerSec + 1.0 / self._KtNMPerAmp * self._rOhms * torqueNm
  ...
  def getNEO(self, numMotors):
    rpm = 5676
    return DCMotor(
        12, 2.6, 105, 1.8, rpm * math.pi / (60.0 / 2.0), numMotors)